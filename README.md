...
| `rust/cmix-rs/` | arith coder, sigmoid, state machines, context manager, Mixer + LSTM stack, SSE smoother, all Context types, Direct/Indirect/Match/Byte/Bracket models, full Shelwien PPMd port, FXCMv1 with full Predictor scaffolding (all 32 ContextMaps + Mixers + APMs + brackets/columns/words/match-model wired) and most modelPrediction byte-boundary sub-steps ported (FxcmState helpers); the per-byte ContextMap feeds + per-bit pipeline are the remaining integration | 121 unit tests | — |

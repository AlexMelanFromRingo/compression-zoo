// test_brotli.cpp — self-contained smoke test for brotli.dll.
//
// Loads brotli.dll via LoadLibrary, calls GetNumberOfMethods /
// GetMethodProperty to verify it advertises method 0x4F71102 "Brotli",
// then instantiates encoder + decoder via CreateObject and round-trips
// a buffer through ICompressCoder::Code in memory.
//
// Build (from plugins/brotli):
//   x86_64-w64-mingw32-g++ -O2 -std=c++14 \
//       tests/test_brotli.cpp -o tests/test_brotli.exe -loleaut32 -luuid
//
// Run on Windows (with build/brotli.dll next to it OR placed in
// 7-Zip's Codecs folder):
...
int main(int argc, char **argv) {
    if (argc < 2) { fprintf(stderr, "usage: %s path-to-brotli.dll\n", argv[0]); return 2; }
    ...
    if (!found) { fprintf(stderr, "method 0x4F71102 not advertised\n"); return 1; }
    fprintf(stderr, "method Brotli found: id=0x%llX name=%s\n", (unsigned long long)methodId, methodName);
    ...
    ICompressCoder *enc = NULL;
    {
        ...
        if (hr != S_OK || !enc) {
            fprintf(stderr, "CreateObject(encoder) failed: hr=0x%08lx\n", (long)hr);
            return 1;
        }
    }
